...
#define SHMPERM SHM_R | SHM_W   /* read 0400 | write 0200 */
...
struct pt_entry {
    ...
    int frame;                  /* Frame idx */
    int count;                  /* Global counter as quasi-timestamp
                                   for LRU */
};
...
struct vmem_adm_struct {
    ...
    int last_alloc_idx;         /* Last frame allocated (FIFO, CLOCK)
                                 */
    int pf_count;               /* Page fault counter */
    int g_count;                /* Global acces counter as
                                   quasi-timestamp */
};
...
struct vmem_struct {
    ...
    struct pt_struct pt;
    int data[VMEM_NFRAMES * VMEM_PAGESIZE];  /*  PHYSMEM  */
};
